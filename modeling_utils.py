...
class PreTrainedModel(nn.Module, ModuleUtilsMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        if not isinstance(config, PretrainedConfig):
            ...
            print("cls.config_class",cls.config_class)#<class 'transformers_copy.configuration_openai.OpenAIGPTConfig'>
        ...
        # Load model
        print("cls.pretrained_model_archive_map",cls.pretrained_model_archive_map)#{'openai-gpt': 'https://s3.amazonaws.com/models.huggingface.co/bert/openai-gpt-pytorch_model.bin'}
        print("WEIGHTS_NAME",WEIGHTS_NAME)#pytorch_model.bin
        if pretrained_model_name_or_path is not None:
            ...
            if resolved_archive_file == archive_file:
                logger.info("loading weights file {}".format(archive_file))#loading weights file gpt_personachat_cache/pytorch_model.bin
            else:
        ...
        # Instantiate model.
        #model_kwargs={}
        print("model_args2",model_args,"model_kwargs2",model_kwargs)#model_args2 () model_kwargs2 {}
        model = cls(config, *model_args, **model_kwargs)
        print("state_dict",state_dict)#state_dict=None
        if state_dict is None and not from_tf:
            try:
                state_dict = torch.load(resolved_archive_file, map_location="cpu")#是一个dict存储着所有的参数和参数值
            except Exception:
                raise OSError(
        ...
        if from_tf:
            ...
            for old_key, new_key in zip(old_keys, new_keys):
                state_dict[new_key] = state_dict.pop(old_key)#替换旧的键值为新的键值

            # copy state_dict so _load_from_state_dict can modify it
            metadata = getattr(state_dict, "_metadata", None)
            state_dict = state_dict.copy()
            print("state_dict的类型是",type(state_dict))#<class 'dict'>
            if metadata is not None:
                print("state_dict有_metadata")#事实上并没有这个属性，所以不会输出
                state_dict._metadata = metadata

            # PyTorch's `_load_from_state_dict` does not copy parameters in a module's descendants
            # so we need to apply the function recursively.
            def load(module: nn.Module, prefix=""):#只是写函数的人提醒用函数的人最好传什么类型的参数
            ...
            start_prefix = ""
            model_to_load = model
            print("cls.base_model_prefix",cls.base_model_prefix)#cls.base_model_prefix transformer
            ...
            print({"missing_keys": missing_keys, "unexpected_keys": unexpected_keys, "error_msgs": error_msgs})
...
class SequenceSummary(nn.Module):
    ...
    def __init__(self, config: PretrainedConfig):
        ...
        self.summary_type = getattr(config, "summary_type", "last")#"cls_index"
        ...
        if hasattr(config, "summary_first_dropout") and config.summary_first_dropout > 0:
            self.first_dropout = nn.Dropout(config.summary_first_dropout)#0.1
    ...
    def forward(self, hidden_states, cls_index=None):
        ...
        if self.summary_type == "last":
            ...
            if cls_index is None:
                ...
                cls_index = cls_index.unsqueeze(-1).unsqueeze(-1)#[4, 2]
                #print("cls_index",cls_index,cls_index.size())
                cls_index = cls_index.expand((-1,) * (cls_index.dim() - 1) + (hidden_states.size(-1),))
                #print("cls_index2",cls_index,cls_index.size())
            # shape of cls_index: (bsz, XX, 1, hidden_size) where XX are optional leading dim of hidden_states
            output = hidden_states.gather(-2, cls_index).squeeze(-2)  # shape (bsz, XX, hidden_size)
            #[bsz, XX, 300, hidden_size]
